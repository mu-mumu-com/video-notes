...
- **本業の定期業務リマインド・消化確認は、ローカルでりかちゃんを呼んだ時だけ**動く（Task 13 のペルソナ側）。`~/SW/work-duties.md` ＋ iPhone リマインダー ＋ Google カレンダーを読む。
- **プライベートの予定は iPhone のリマインダー**（Google カレンダーではない）。朝のショートカットは**本業用リスト（例『仕事』）を除外**して個人リマインダーだけを送る（Task 12）。
...
- **Task 13**: `agent/secretary/CLAUDE.md` に「ローカル呼び出し時は `~/SW/work-duties.md`・iPhone リマインダー（`shortcuts` CLI か AppleScript）・Google カレンダーを読んで、本業（平日のみ）を含むフルブリーフィングをする」を明記。
